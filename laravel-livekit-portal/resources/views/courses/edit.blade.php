...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6 flex items-center gap-3">
        ...
        <h1 class="text-2xl font-extrabold tracking-tight">Edit Course</h1>
    </div>
        ...
        <form method="POST" action="{{ route('courses.update', $course) }}" class="space-y-6">
            ...
            <div>
                ...
                <select id="category_id" name="category_id" required
                       class="block w-full rounded-lg border @error('category_id') border-red-500 @else border-gray-600 @enderror bg-gray-700 px-4 py-3 text-white focus:border-indigo-500 focus:ring-indigo-500 text-sm transition">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="block w-full rounded-lg border @error('description') border-red-500 @else border-gray-600 @enderror bg-gray-700 px-4 py-3 text-white placeholder-gray-500 focus:border-indigo-500 focus:ring-indigo-500 text-sm transition resize-none">{{ old('description', $course->description) }}</textarea>
                @error('description')
                    <p class="mt-1.5 text-xs text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="flex items-center rounded-lg border @error('room_name') border-red-500 @else border-gray-600 @enderror bg-gray-700 overflow-hidden focus-within:border-indigo-500 transition">
                    ...
                    <input type="text" id="room_name" name="room_name" value="{{ old('room_name', $course->room_name) }}"
